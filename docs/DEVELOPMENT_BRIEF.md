...
**适用对话**：`02_软件开发`
**项目总控**：`01_项目总控与决策`
**当前状态**：M1已由02在本地交付并由01完成初步审查，尚未正式合并；本文件是经若翎确认后的M2执行版。M2采用“一个工作空间多企业、一次评估运行一企业、支持对比和独立报告”，外部模型负责会话调度，多文件范围和DMG/MSI交付形态不变。
...
02开始M1整改或M2实现前必须同时满足：

1. 若翎在`02_软件开发`对话中发送完全一致的口令“开始开发MVP”；
2. 02重新读取本文件v0.5及最新`PROJECT_PLAN.md`、`PROJECT_STATUS.md`、`DATA_STATUS.md`、`DATA_PROFILE.md`、`DATA_CONTRACT.md`和`DECISIONS.md`；
3. 保留当前M1本地成果和未提交修改，不重置、不覆盖，不把原始Excel、ZIP、运行缓存或构建产物纳入Git；
4. 先完成第14节M1强制整改并通过扩展测试和真实工作簿回归，再进入桌面封装；
5. 按第25节默认值实施未确认的小项，任何涉及正式评分、模型效果、第二深验行业或MVP范围的变化必须回到01和若翎确认；
6. Windows构建必须具有真实Windows、Windows虚拟机或Windows CI环境；若尚未具备，只能把Windows状态标记为“待构建/待验证”。

## 13. M1复审仍需若翎确认的问题

1. 是否接受01对M1“主流程条件性通过、整改后合并”的审查结论。
...
02已在本地工作树交付M1代码、测试、运行说明和必要的`.gitignore`调整，尚未提交或推送。01已复跑单元/API测试、真实配套工作簿和实际网页闭环；正常路径能够完成上传、五表校验、1000家企业关联、企业详情、年度变化、质量提示、目录候选、参考层展示和Markdown报告。

M1当前结论为“主流程条件性通过，整改后方可正式合并”。02在下一阶段UI与桌面封装前必须先完成：
...
上述整改通过自动化测试和真实工作簿回归后，M1才可由01正式合并。不得为了开始UI工作删除这些整改项。
...
| 多行业差异化适配 | 行业路由、8行业配置框架、行业模板隔离、2行业深验 | 完整MVP范围已确认；铜为第一深验，第二行业待确认 |
...
| 8. 行业路由 | 查看所属行业和适用规则 | 选择行业模板、目录版本、标准/因子版本 | 8行业框架；深验状态如实标注 |
...
02开始执行前应读取本文件v0.5和最新`PROJECT_PLAN.md`、`PROJECT_STATUS.md`、`DATA_STATUS.md`、`DATA_PROFILE.md`、`DATA_CONTRACT.md`、`DECISIONS.md`。阶段交付至少包括：

- 新增/修改文件清单和架构说明；
- 每项验收标准的通过、失败或未验证状态；
- 单元、API、浏览器、macOS和Windows测试命令及摘要；
