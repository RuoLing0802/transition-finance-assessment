...
**版本**：v1.14
**日期**：2026-08-22
**状态**：总体方案已确认；M1—M5已正式集成；11行业统一覆盖已确认；M6专项开发说明已形成，等待独立02 M6任务获得精确口令授权；Windows和报告扩展后置
**项目负责人/主要软件开发者/技术集成人**：若翎
**计划来源**：用户确认的总体方案、2026-08-09精简协作决策、2026-08-21行业范围与知识资产准入决策、2026-08-22补充材料接收及M5正式集成决策
**项目阶段**：M1—M5完成；M6活动数据标准化与碳核算基础层策划完成、软件开发尚未启动
...
截至2026-08-22，当前正式基线实际使用React、TypeScript、Vite、Ant Design、ECharts前端，Python/FastAPI/Pydantic后端，原生`sqlite3`持久化，`openpyxl`及PDF/DOCX/图片解析适配，OpenAI兼容会话模型，LangGraph状态运行时、LangChain Core确定性节点包装、SQLite/FTS5知识检索、PyWebView和PyInstaller。SQLAlchemy、PostgreSQL、正式碳核算、行业基准和评分引擎均尚未接入。目标架构与已接入架构不得混写。
...
后续开发顺序为：M6活动数据与碳核算 → M7行业对标与转型行为 → M8评分与信贷支持 → 报告扩展、Windows与正式分发。该顺序不改变11行业统一覆盖范围。M6专项开发说明已经形成，但仍须若翎在新02 M6任务发送完全一致的开发口令后才能启动，不因M5集成或因子来源入口存在而自动实施。

### 5.9 第五开发里程碑（M5可追溯知识检索，已完成）
...
M5建设了带来源、版本、生效期、条款/表格/页码定位、状态和更新机制的知识检索层。政策知识可作为受控全局候选复用，查询上下文、检索结果和切片读取绑定当前`assessment_run_id`、企业、线程、行业与冻结索引。检索结果只提供候选证据，不修改企业事实、不执行碳核算、不计算评分、不产生授信结论，也不接收`转型规划结论`或其派生文本。国家因子库在M5只登记来源和适用性元数据；因子值冻结、版本快照和计算调用留待M6单独审查。

M5采用记录级白名单和四级可见性，已实现SQLite FTS5确定性本地检索、关键词降级、运行级索引冻结、检索日志、候选证据引用、只读Agent工具和离线降级；不直接引入RAGFlow。终审复验为专项`16 passed`，Gold`33 passed / 11 correct_degrade / 0 failed`，并覆盖受控PDF缺失时的安全降级；详见[`M5_DEVELOPMENT_REVIEW.md`](M5_DEVELOPMENT_REVIEW.md) v1.0。12号731条候选记录没有整表进入普通索引。

### 5.10 第六开发里程碑（M6活动数据与碳核算，策划完成）

M6以[`M6_DEVELOPMENT_PROMPT.md`](M6_DEVELOPMENT_PROMPT.md) v1.0为正式执行依据。M6-A建设2024—2025活动数据宽转长、单位与缺失状态、因子注册/准入/快照、Decimal确定性计算、不可计算状态、证据链、运行隔离和右侧检查器；允许仅在测试中使用隔离的`TEST_ONLY`因子。M6-B只有在03形成条目级因子冻结包并经01确认后，才能用生产因子生成数值结果。

当前生产因子自动调用仍为0条。没有适用因子、组织/设施边界、煤种/热值、购热口径或过程排放活动数据时，系统必须输出`not_calculable`、`partially_calculated`或`needs_review`，不能用0、最近年份、全国碳市场因子或知识检索文本替代。M6不进入M7行业对标、M8评分/信贷、Windows或报告扩展。

## 6. 团队实施方案

### 6.1 若翎的固定Codex对话

项目长期只保留少量职责分区。日常小任务不强制建立编号对话；软件开发可按M5维护、M6开发等主要里程碑建立独立任务，减少长上下文污染：

| 对话 | 当前状态 | 职责边界 |
|---|---|---|
| `00_项目上下文整理与Codex交接包` | 已完成，可归档 | 仅保留早期上下文整理成果，不承担后续日常管理 |
| `01_项目总控与决策` | 当前主对话 | 维护本计划、重大决策和项目状态；审查成员成果；经若翎确认后正式合并、提交和推送；不承担日常软件编码 |
| `02_软件开发`任务族 | M1—M5已正式集成；M6待授权 | 原M5任务用于M5维护；新M6任务按`DEVELOPMENT_BRIEF.md` v1.0和`M6_DEVELOPMENT_PROMPT.md` v1.0实施。各任务不得并行修改同一正式工作树，成果交01审查，不自行提交或推送 |
| `03_数据、政策与评分体系` | 知识治理已支持M5，继续准备M6因子冻结包 | 负责正式数据导入与审计、政策标准、行业规则、条目级因子准入、指标体系和评分校准；形成结论后由`01`更新正式计划、决策与状态 |
