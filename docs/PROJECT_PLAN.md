...
**计划来源**：用户提供的总体计划，已整理为仓库内普通Markdown文档
**项目阶段**：材料审阅与总体规划阶段；PM-001治理文档收尾完成，开发未授权

**开发门槛**：只有收到完全一致的口令“开始开发MVP”后才进入程序开发；本计划当前不授权编程。

> 本文件是当前总体方案的可移植版本。所有路径均使用仓库相对路径；原计划中的Codex专用文件引用标记已改为普通Markdown链接。
...
若翎负责技术架构、核心程序、前后端整合、评估引擎、桌面封装、部署和最终集成。吴、钟、刘、夏不固定绑定某一工作方向，而是按 `TASK_BOARD.md` 的任务卡动态参与政策标准、评分规则、数据治理、软件辅助、测试、报告和演示。每个明确成果使用独立Codex对话，并按 [`TEAM_ROLES.md`](TEAM_ROLES.md) 的格式交付。
...
获得开发授权后，可按需新增或维护：`docs/DATA_CONTRACT.md`、`docs/SCORING_RULEBOOK.md`、`docs/ARCHITECTURE.md`、`docs/TEAM_PLAN.md`、`docs/TEST_PLAN.md`、`docs/RISK_REGISTER.md`、`docs/SUBMISSION_CHECKLIST.md`。本轮不启动开发；PM-001治理收尾允许若翎的项目统领Codex提交本地Git，正式远端推送仅在远端已配置并经若翎确认后进行。
