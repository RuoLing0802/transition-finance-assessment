...
**版本**：v1.11
**日期**：2026-08-21
**状态**：总体方案已确认；M1—M4已正式集成；11行业统一覆盖已确认；03知识资产整改后再进入M5策划；Windows和报告扩展后置
**项目负责人/主要软件开发者/技术集成人**：若翎
**计划来源**：用户确认的总体方案、2026-08-09精简协作决策及2026-08-21行业范围与知识资产准入决策
...
后续开发顺序为：M5知识检索 → M6碳核算 → M7行业对标与转型行为 → M8评分与信贷支持 → 报告扩展、Windows与正式分发。该顺序不改变11行业统一覆盖范围。M5必须另行形成开发说明并经若翎确认，不因M4集成或知识材料到达自动启动。

### 5.9 M5材料准备状态

钟同学原始交付和03第二阶段知识资产已经归档。01终审确认数量闭合、CETS主映射、原始数据哈希、个人路径清理和泄漏/自动调用边界，但09行业包、12号语料75条非目录记录、13号跨表测试引用、14号文献引用字段以及08/10部分来源状态仍须整改。整改完成并复审通过前，只作为候选研究资产，不批准02实施M5；新增因子允许自动调用仍为0条。
...
| `02_软件开发` | M1—M4已正式集成；M5待策划 | 按`DEVELOPMENT_BRIEF.md` v0.8和后续阶段说明实施；M5不得提前实现碳核算、评分、Windows或报告扩展；成果交给01审查，不自行提交或推送 |
