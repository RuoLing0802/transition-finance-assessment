...
**版本**：v1.7
**日期**：2026-08-16
**状态**：总体方案已确认；M1已由02在本地交付并经01初审，整改与正式合并待完成；若翎已确认M2采用“一个工作空间多企业、一次评估运行一企业、支持对比和独立报告”，三端交付方案继续有效
**项目负责人/主要软件开发者/技术集成人**：若翎
**计划来源**：用户确认的总体方案与2026-08-09精简协作决策
**项目阶段**：M1成果审查与下一阶段开发规划阶段；M1尚未正式合并
...
已确认的“8行业框架＋2行业深验”是总体MVP边界，不属于可削减项。铜产业是第一深验行业；第二深验行业待配套数据审计、正式标准和评审需要进一步确认后，按样本量、规范完整度、评委可理解性和减排金融价值确定。
...
### 5.5 第一开发里程碑（本地交付，待整改与合并）

`docs/DEVELOPMENT_BRIEF.md` 是 `02_软件开发` 的当前执行依据。02已在本地交付“Excel上传 → 五张表结构校验 → 按企业代号关联 → 企业详情展示 → 2024—2025年能耗变化分析 → 缺失数据提示 → `转型目录`匹配 → 基础评估报告 → 与`转型规划结论`对照验证”的主流程；01初审结论为条件性通过，异常输入、批次质量汇总、参考对照语义、重复性和安全测试整改后方可正式合并。

该里程碑不改变完整“8行业框架＋2行业深验”MVP范围，也不提前确定评分权重、阈值、模型、第二深验行业或最终业务规则。M1本地成果、运行缓存和原始数据均尚未进入正式仓库。

### 5.6 第二开发里程碑（方案已形成，实施由02负责）

M2按照 [`DEVELOPMENT_BRIEF.md`](DEVELOPMENT_BRIEF.md) v0.5实施：先完成M1整改门，再建立多企业工作空间和单运行隔离的Codex式评估工作台。外部模型调度当前评估运行的会话和受控工具，规则/核算/评分保持独立可回放；同期支持XLSX、原生/扫描PDF、DOCX和图片；同一套React/FastAPI业务通过Web、macOS `.app`/DMG和Windows `.exe`/MSI运行。M2执行顺序固定为M1整改→领域模型与持久化→多企业Web交互→多模态解析→模型编排→报告/对比→桌面封装。

1000企业工作簿作为可复用数据批次导入，当前评估运行必须选择并绑定唯一企业；同一工作空间评估其他企业时创建新的运行并复用批次。对话回答、工具执行和报告只归入当前运行，发现跨企业冲突时阻断。已完成运行可生成企业对比视图，但不同期间、规则版本或口径不一致时标记不可直接比较。外部模型不可用时，离线固定流程仍完成基础解析、质量、查询、目录和报告闭环。Windows成品必须在Windows环境构建和验收，DMG/MSI必须分别完成安装测试。M2不直接确定正式评分、碳核算、模型效果或授信结论。
...
| `02_软件开发` | M1本地交付、待整改与正式合并；M2开发指导已形成 | 负责M1整改、多企业工作空间与单评估运行隔离、外部模型会话编排、多模态解析、Codex式前端、数据库、测试、DMG/MSI和部署；成果交给01审查，不自行提交或推送 |
...
配套模拟数据接收登记和初步审计完成后，数据契约确认、字段语义、质量规则、政策标准、行业规则和评分校准仍主要在 `03_数据、政策与评分体系` 进行；`03` 的长期结论由 `01` 纳入正式文件。`转型规划结论`只能作为参考结论和验证对照，不能回流为同一模型输入特征。软件开发始终在 `02_软件开发` 进行；只有收到完全一致的口令“开始开发MVP”后才允许编程。当前不改变“8行业框架＋2行业深验”范围。
