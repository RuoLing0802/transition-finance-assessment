...
**版本**：v1.1
**日期**：2026-08-15
**状态**：总体方案已确认；命题方配套模拟数据已登记，完整数据审计、独立字典/标签/划分和标准核验待完成
...
准入门、权重、阈值、行业基准、等级边界和补全规则均属于**待数据审计、独立数据字典、正式评分标签和命题方标准确认的专业判断初稿**。配套模拟数据进入分析或开发前，必须先完成文件盘点、字段语义、质量、缺失/异常/重复、口径与单位、标签泄漏和可用性分级审计；取得独立标签、训练/测试划分和正式标准后，再重新校准。
...
配套模拟数据已接收登记后，数据盘点、字段审计、质量检查、规则研究和评分校准主要在 `03_数据、政策与评分体系` 进行；`03` 的长期结论由 `01` 纳入正式文件。`转型规划结论`只能作为参考结论和验证对照，不能回流为同一模型输入特征。软件开发始终在 `02_软件开发` 进行；只有收到完全一致的口令“开始开发MVP”后才允许编程。当前不改变“8行业框架＋2行业深验”范围。
