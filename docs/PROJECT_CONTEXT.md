...
**版本**：v1.1
**日期**：2026-08-16
**状态**：M1已由02在本地交付并经01初步审查，尚未正式合并；M2已确认采用多企业工作空间、单评估运行一企业、企业对比和独立报告，外部模型会话编排、多模态输入和DMG/MSI方案继续有效
...
02的软件开发当前以 [`DEVELOPMENT_BRIEF.md`](DEVELOPMENT_BRIEF.md) v0.5 M2执行版为下一阶段依据；01只负责方案、审查、正式合并和Git操作，不承担日常编码。

M1已由02在本地交付，覆盖配套Excel上传、五表校验、企业代号关联、详情展示、2024—2025能耗变化、缺失提示、目录匹配、基础报告和参考结论隔离。01复核认为主流程可以跑通，但仍存在失败批次后续查询异常、批次汇总未聚合企业级警告、参考结论对照偏弱、完整报告不可重复和负向测试不足等问题。因此M1只能记为“有条件通过、待整改”，不能写成已经正式合并或验收完成。

若翎已确认M2采用“一个工作空间多企业、一次评估运行一企业”的Codex式评估工作台：左侧显示工作空间及其评估运行，右侧是由外部模型编排的对话，底部支持XLSX、PDF、DOCX和图片上传。1000企业工作簿先作为可复用批次校验，再为当前评估运行选择唯一企业；同一工作空间的其他企业创建新运行并复用批次，所有材料不得跨运行混入。已完成运行可生成企业对比视图。模型选择器选择实际会话调度模型，规则/评分配置独立展示。三端共享React/FastAPI；macOS交付`.app`和DMG，Windows交付`.exe`和MSI，并在对应系统中实际构建、安装和测试。
...
精确开发口令“开始开发MVP”继续作为启动新开发实施的权限边界；“继续执行”“按照计划开展”或“实施计划”均不能替代该口令。当前已经存在的M1本地交付仅按事实登记，其整改、M2实现、桌面封装和部署均由02根据最新版开发指导执行，01不直接编写程序。
...
- 已完成：02已在本地交付M1，01复跑了测试、真实配套模拟工作簿和浏览器主流程；M1主闭环可运行，但审查结论为“有条件通过、待整改”，尚未正式合并或推送。
- 已完成：[`DEVELOPMENT_BRIEF.md`](DEVELOPMENT_BRIEF.md) 已更新至v0.5 M2执行版，明确多企业工作空间、单评估运行一企业、企业对比、独立报告、领域模型、接口、LangChain/LangGraph采用边界、执行顺序、工具边界、离线回退、DMG/MSI和端到端业务流程。
- 下一步：若翎在02发送精确口令后，由`02`先完成M1强制整改，再实施工作空间与评估运行域、多模态解析、模型编排、React界面、报告库、企业对比及DMG/MSI；由`01`进行里程碑审查和正式集成。`03`并行继续核验数据字典、字段口径、标签、目录来源、排放因子和折标系数。
- 尚未开始或尚未完成：M1正式合并、M2三端实现、正式评分校准、碳核算、OCR、模型效果验证、生产部署和竞赛最终材料制作。
