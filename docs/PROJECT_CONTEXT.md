...
在数据到达前，可以产出数据需求、接口契约、质量规则和校准方案；收到“开始开发MVP”口令后，可推进架构、解析、规则引擎、报告、网页、桌面和部署工程。如需构造最小测试样例，仍须先说明生成规则并取得若翎确认，且显式标注“模拟数据，不代表比赛正式数据”；测试样例不得用于证明正式比赛效果。数据到达后的审计流程见 [`DATA_STATUS.md`](DATA_STATUS.md)。
...
- 下一步：成员更新仓库并读取最新版核心文档；等待正式数据、政策标准材料和精确开发口令，根据实际任务推进 `03_数据、政策与评分体系` 或 `02_软件开发`。
