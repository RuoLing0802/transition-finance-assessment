...
**版本**：v0.7
**日期**：2026-08-15
**状态**：总体规划成果已形成，02开发指导已发布，但阶段锁仍处于规划阶段；开发未授权（只有精确口令“开始开发MVP”才可开启）
...
02的软件开发当前以 [`DEVELOPMENT_BRIEF.md`](DEVELOPMENT_BRIEF.md) 为授权后的执行依据。第一里程碑先打通配套Excel的上传、五表校验、企业代号关联、详情展示、2024—2025能耗变化、缺失提示、目录匹配、基础报告和参考结论对照验证；该里程碑不改变完整“8行业框架＋2行业深验”MVP范围。未收到完全一致的口令“开始开发MVP”前，仍不得编写程序或启动软件开发。
...
- 下一步：由`03`继续核验独立数据字典、字段口径、标签定义、训练/测试划分、正式评分标签、目录来源/版本/生效期、组织与核算边界、排放因子和折标系数；由若翎确认数据契约候选规则。
- 尚未开始：根据确认契约进行程序实现、正式评分校准、模型训练、系统测试、桌面封装和竞赛最终材料制作。
- 当前缺口：独立数据字典、训练/测试划分、正式评分标签、标准全文、排放因子和部分行业资料未齐；宽转长、缺失等级、质量阻断、目录`rule_id`、评分权重、阈值、模型、第二深验行业和MVP范围待确认。这些缺口影响校准与后续实现，但不改变当前阶段锁或既有MVP范围。
