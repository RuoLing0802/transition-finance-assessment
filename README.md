...
- 正式总体计划：[`docs/PROJECT_PLAN.md`](docs/PROJECT_PLAN.md) v1.1，是本项目唯一正式总体计划。
...
当前只允许材料分析、总体规划、规则需求、数据需求、测试设计、报告大纲和只读评审，不编写程序、不安装依赖、不修改开发环境、不使用模拟数据产出项目结论。收到精确口令“开始开发MVP”后，可以使用已登记的命题方配套模拟数据推进工程，但仍不得把它当作真实企业业务数据或正式评分标签；如需构造额外测试样例，仍须先说明生成规则并取得若翎确认，且显式标注“模拟数据，不代表比赛正式数据”。正式数据、隐私或保密不是工程开发的前置条件。
